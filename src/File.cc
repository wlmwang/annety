...
void File::do_initialize(const StringPiece& path, uint32_t flags) {
  DCHECK(!is_valid());
  ...
  int descriptor = HANDLE_EINTR(::open(path.as_string().c_str(), open_flags, mode));
  ...
  if (flags & FLAG_OPEN_ALWAYS) {
    if (descriptor < 0) {
      ...
      descriptor = HANDLE_EINTR(::open(path.as_string().c_str(), open_flags, mode));
      if (descriptor >= 0) {
        created_ = true;
      }
    }
...
File::File(const StringPiece& path, uint32_t flags)
...
void File::initialize(const StringPiece& path, uint32_t flags) {

...
public:
	...
	inline bool out_of_addressable_space() const {
	...
	inline ssize_t get_count() const {
...
private:
	...
	inline bool increment_count(size_t inc) {
	...
	inline bool increment_count_by_one() {
	...
	inline char* get_insertion_point() const {
		size_t idx = count_;
		if (idx > size_) {
			idx = size_;
		}
...
ssize_t safe_snprintf(char* buf, size_t sz, const char* fmt, const Arg* args,
	...
	size_t padding;
	char pad;
	for (unsigned int cur_arg = 0; *fmt && !buffer.out_of_addressable_space(); ) {
		if (*fmt++ == '%') {
			padding = 0;
			pad = ' ';
			char ch = *fmt++;

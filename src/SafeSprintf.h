...
// safe_sprintf() is a type-safe and completely self-contained version of
// snprintf().
//
// safe_snprintf() is an alternative function signature that can be used when
// not dealing with fixed-sized buffers. When possible, safe_sprintf() should
// always be used instead of safe_snprintf()
//
...
// and safe_sprintf() instead returns an output string that expands only
...
//   %c, %o, %d, %x, %X, %p, and %s.
//
// safe_sprintf() aims to be as liberal as reasonably possible. Integer-like
...
// safe_sprintf() does not support at this time.
//
...
// have little impact on other parts of safe_sprintf() function.
//
...
//
// safe_sprintf() and safe_snprintf() mimic the behavior of snprintf() and they
...
// like safe_sprintf(buf, "%p %d", 1, 2) results in "%p 2"). See above for
// the use of RAW_CHECK() in debug builds, though.
//
// Example:
//   char buf[20];
//   strings::safe_sprintf(buf, "The answer: %2d", 42);
//
...
// limited and known. On the other hand, if the parameters to safe_snprintf()
...
//   for (;;) {
//     char buf[size];
//     size = safe_snprintf(buf, size, "Error message \"%s\"\n", err) + 1;
...
#ifndef ANT_SAFE_SPRINTF_H_
#define ANT_SAFE_SPRINTF_H_
...
ssize_t safe_snprintf(char* buf, size_t sz, const char* fmt,
	...
	void set_safe_sprintf_ssize_max_for_test(size_t max);
	size_t get_safe_sprintf_ssize_max_for_test();
#endif

}	// namespace internal

template<typename... Args>
ssize_t safe_snprintf(char* buf, size_t N, const char* fmt, Args... args) {
	// Use Arg() object to record type information and then copy arguments to an
	// array to make it easier to iterate over them.
	const internal::Arg arg_array[] = { args... };
	return internal::safe_snprintf(buf, N, fmt, arg_array, sizeof...(args));
}

template<size_t N, typename... Args>
ssize_t safe_sprintf(char (&buf)[N], const char* fmt, Args... args) {
	// Use Arg() object to record type information and then copy arguments to an
	// array to make it easier to iterate over them.
	const internal::Arg arg_array[] = { args... };
	return internal::safe_snprintf(buf, N, fmt, arg_array, sizeof...(args));
...
ssize_t safe_snprintf(char* buf, size_t N, const char* fmt);

template<size_t N>
inline ssize_t safe_sprintf(char (&buf)[N], const char* fmt) {
	return safe_snprintf(buf, N, fmt);
}

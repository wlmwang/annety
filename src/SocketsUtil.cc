...
int shutdown(int servfd, int how)
{
	DCHECK(servfd >= 0);

	int ret = ::shutdown(servfd, how);
...
// The options SO_RCVTIMEO and SO_SNDTIMEO will only affect the socket functions such as send, 
// sendmsg, recv, recvmsg.
// The system will not stop retransmission after timeout, and will not mark the socket as an 
// error. After the options are set, if they timeout, return -1 and set errno to EAGAIN or 
// EWOULDBLOCK.

// SO_RCVTIMEO and SO_SNDTIMEO Can only be used for read and write operations, not valid for 
// connect. -- From UNP.
// But it works on Linux.  SO_RCVTIMEO for accept(), SO_SNDTIMEO for connect().
...
int set_snd_timeout(int servfd, double time_s)
{
	...
	TimeDelta td = TimeDelta::from_seconds_d(time_s);
	struct timeval tv = td.to_timeval();

	socklen_t optlen = static_cast<socklen_t>(sizeof tv);

	int ret = setsockopt(servfd, SOL_SOCKET, SO_SNDTIMEO, &tv, optlen);
	PCHECK(!ret);

	return ret;
#else
	LOG(ERROR) << "SO_SNDTIMEO is not supported.";
...
int set_rcv_timeout(int servfd, double time_s)
{
	...
	int ret = setsockopt(servfd, SOL_SOCKET, SO_RCVTIMEO, &tv, optlen);
	PCHECK(!ret);

	return ret;
#else
	LOG(ERROR) << "SO_RCVTIMEO is not supported.";
...
int set_keep_alive(int servfd, bool on)
{
	...
	int ret = ::setsockopt(servfd, SOL_SOCKET, SO_KEEPALIVE, &opt, optlen);
	PCHECK(!ret);
...
int set_tcp_nodelay(int servfd, bool on)
{
	...
	int ret = ::setsockopt(servfd, IPPROTO_TCP, TCP_NODELAY, &opt, optlen);
	PCHECK(!ret);
...
int get_sock_error(int servfd)
{
	...
	if (::getsockopt(servfd, SOL_SOCKET, SO_ERROR, &err, &optlen) < 0) {
		return errno;
	}
...
struct sockaddr_in6 get_local_addr(int servfd)
{
	...
	int ret = ::getsockname(servfd, sockaddr_cast(&addr), &addrlen);
...
struct sockaddr_in6 get_peer_addr(int servfd)
{
	...
	// The kernel will fill in the value of `addrlen` and `addr`.
	int ret = ::getpeername(servfd, sockaddr_cast(&addr), &addrlen);

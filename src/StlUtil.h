...
// Definitions for size()/empty()/data()------------------------
...
constexpr bool empty(const T (&array)[N]) noexcept {
	return false;	// Note, cannot determination
}
template <typename T>
...
// Definitions for STLCount()/ContainsKey()/ContainsValue()----------------
...
// Definitions for STLSetDifference/STLSet*----------------------------------
...
// for {,unordered_,multi,...}map/{,unordered_,multi,...}set
namespace internal {
...
template <class Key,
		  ...
		  class Allocator,
		  class Predicate>
void EraseIf(std::unordered_multimap<Key, T, Hash, KeyEqual, Allocator>& container,
			 Predicate pred) {
	internal::IterateAndEraseIf(container, pred);
}
